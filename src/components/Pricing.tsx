      ...
      {(["monthly", "annual"] as Billing[]).map((b) => {
        ...
        return (
          <button
            ...
            onClick={() => onChange(b)}
            className={`relative flex items-center gap-2 rounded-xl px-5 py-2 text-xs font-semibold tracking-wide transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-600/60 ${
              active
  ...
  return (
    <div
      ...
      className={`group/card relative flex flex-col rounded-2xl border p-6 transition-[border-color,box-shadow,transform] duration-500 motion-reduce:transition-none lg:p-7 ${
        plan.popular
          ? "border-red-700/60 bg-red-950/20 shadow-[0_0_40px_rgba(220,38,38,0.2)] lg:scale-[1.03]"
          : "glass-card border-white/8 hover:border-red-800/40 hover:shadow-[0_0_28px_rgba(220,38,38,0.15)]"
    ...
    >
      {plan.popular && (
        <div className="absolute -top-3.5 left-1/2 -translate-x-1/2">
          <Badge
            variant="neon"
            className="px-3 py-1 text-[9px] font-bold tracking-[0.18em] uppercase shadow-[0_0_14px_rgba(220,38,38,0.4)]"
          >
            Mais popular
          </Badge>
        </div>
      ...
      <ul className="mb-7 flex flex-col gap-2.5" role="list" aria-label={`Itens incluídos no plano ${plan.name}`}>
        {plan.features.map((feature) => (
          <li key={feature} className="flex items-start gap-2.5">
            <Check
              size={15}
              className="mt-0.5 shrink-0 text-red-600 drop-shadow-[0_0_4px_rgba(220,38,38,0.6)]"
              aria-hidden
            />
...
export function Pricing() {
  ...
  return (
    ...
    >
      <div
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_70%_55%_at_50%_0%,rgba(220,38,38,0.07)_0%,transparent_55%)]"
        aria-hidden
      />
      <div
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 lg:px-12">
        ...
        >
          ...
          >
            ...
            <span className="block font-display text-[clamp(2rem,5.5vw,4rem)] leading-[0.9] tracking-[0.02em] text-red-600 text-glow-red">
              INVISTA EM VOCÊ.
            </span>
          </h2>
        ...
        <div
          ...
          role="list"
          aria-label="Planos da Academia Garrote"
        >
