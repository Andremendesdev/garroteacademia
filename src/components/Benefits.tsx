"use client";

import { useEffect, useRef, useState } from "react";
import {
  Dumbbell,
  TrendingUp,
  Flame,
  Zap,
  Heart,
  Target,
  type LucideIcon,
} from "lucide-react";
import { Badge } from "@/components/ui/badge";

const benefits: { icon: LucideIcon; title: string }[] = [
  { icon: Dumbbell, title: "Equipamentos Profissionais" },
  { icon: TrendingUp, title: "Ganho de Massa Muscular" },
  { icon: Flame, title: "Emagrecimento e Definição" },
  { icon: Zap, title: "Mais Energia no Dia a Dia" },
  { icon: Heart, title: "Saúde e Qualidade de Vida" },
  { icon: Target, title: "Acompanhamento Especializado" },
];

function BenefitCard({
  icon: Icon,
  title,
  index,
  visible,
}: {
  icon: LucideIcon;
  title: string;
  index: number;
  visible: boolean;
}) {
  const delayClass = `delay-${Math.min((index + 2) * 100, 700)}`;

  return (
    <div
      role="listitem"
      className={`group/benefit glass-card flex items-center gap-3 rounded-xl border border-white/8 p-3.5 transition-[border-color,box-shadow] duration-500 hover:border-red-800/45 hover:shadow-[0_0_28px_rgba(220,38,38,0.18)] motion-reduce:transition-none sm:gap-3.5 sm:p-4 lg:p-4 ${
        visible ? `animate-scale-in ${delayClass}` : "opacity-0"
      }`}
    >
      <div
        className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-red-800/30 bg-red-950/40 shadow-[0_0_14px_rgba(220,38,38,0.18)] transition-shadow duration-500 group-hover/benefit:border-red-700/50 group-hover/benefit:shadow-[0_0_20px_rgba(220,38,38,0.3)] sm:h-10 sm:w-10 sm:rounded-xl"
        aria-hidden
      >
        <Icon size={18} className="text-red-600 drop-shadow-[0_0_6px_rgba(220,38,38,0.7)] sm:h-5 sm:w-5" />
      </div>
      <h3 className="text-xs font-semibold leading-snug tracking-wide text-zinc-100 sm:text-sm lg:text-[0.9rem]">
        {title}
      </h3>
    </div>
  );
}

export function Benefits() {
  const sectionRef = useRef<HTMLElement>(null);
  const [visible, setVisible] = useState(false);

  useEffect(() => {
    const el = sectionRef.current;
    if (!el) return;

    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          setVisible(true);
          observer.disconnect();
        }
      },
      { threshold: 0.1 }
    );

    observer.observe(el);
    return () => observer.disconnect();
  }, []);

  return (
    <section
      ref={sectionRef}
      id="beneficios"
      aria-labelledby="benefits-heading"
      className="relative overflow-x-clip bg-[var(--bg-deep)] py-10 sm:py-12 lg:py-16"
    >
      <div
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_80%_60%_at_50%_100%,rgba(220,38,38,0.07)_0%,transparent_55%)]"
        aria-hidden
      />
      <div
        className="ambient-orb pointer-events-none absolute -left-32 top-1/3 h-[300px] w-[300px] rounded-full opacity-30"
        aria-hidden
      />
      <div className="grain-overlay pointer-events-none absolute inset-0 z-0 opacity-20" aria-hidden />

      <div className="relative z-10 mx-auto max-w-7xl px-6 lg:px-12">
        <div className="mb-6 max-w-2xl sm:mb-8 lg:mb-10">
          <Badge
            variant="neon"
            className={`mb-3 w-fit gap-1.5 px-2.5 py-1 text-[8px] font-semibold tracking-[0.18em] uppercase sm:mb-4 sm:text-[9px] ${
              visible ? "animate-slide-up" : "opacity-0"
            }`}
          >
            <span
              className="glow-pulse h-1 w-1 rounded-full bg-red-600 shadow-[0_0_6px_rgba(220,38,38,0.9)]"
              aria-hidden
            />
            Benefícios
          </Badge>

          <h2
            id="benefits-heading"
            className={`space-y-0 leading-none ${
              visible ? "animate-slide-up delay-100" : "opacity-0"
            }`}
          >
            <span className="block font-display text-[clamp(1.75rem,5vw,3.25rem)] leading-[0.9] tracking-[0.02em] text-[var(--white-neon)]">
              MAIS QUE UMA ACADEMIA.
            </span>
            <span className="block font-display text-[clamp(1.75rem,5vw,3.25rem)] leading-[0.9] tracking-[0.02em] text-red-600 text-glow-red">
              UM NOVO ESTILO DE VIDA.
            </span>
          </h2>

          <div
            className={`mt-3 flex items-center gap-3 sm:mt-4 ${
              visible ? "animate-slide-up delay-200" : "opacity-0"
            }`}
            aria-hidden
          >
            <div className="neon-red-line h-px w-10 bg-red-600/80" />
            <div className="neon-red-line h-[3px] w-[3px] rounded-full bg-red-600 shadow-[0_0_8px_rgba(220,38,38,0.9)]" />
            <div className="h-px w-4 bg-red-700/50" />
          </div>
        </div>

        <div
          className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 sm:gap-3 lg:grid-cols-3 lg:gap-4"
          role="list"
          aria-label="Benefícios da Academia Garrote"
        >
          {benefits.map(({ icon, title }, index) => (
            <BenefitCard
              key={title}
              icon={icon}
              title={title}
              index={index}
              visible={visible}
            />
          ))}
        </div>
      </div>
    </section>
  );
}
